...
namespace
{
    ...
    void test_single_event(ventura::absolute_path const &watched_dir, std::function<void()> const &provoke_event,
                           std::function<void(ventura::file_notification const &)> const &on_event)
    {
        ...
        auto consumer = Si::consume<Si::error_or<ventura::file_notification>>(
            [&io, &got_event, &on_event](Si::error_or<ventura::file_notification> const &event)
            {
                io.stop();
                on_event(event.get());
                got_event = true;
...
BOOST_AUTO_TEST_CASE(file_system_watcher_add_regular)
{
    ...
    test_single_event(watched_dir, std::bind(touch, test_file), [&test_file](ventura::file_notification const &event)
                      {
...
BOOST_AUTO_TEST_CASE(file_system_watcher_change_metadata)
{
    ...
    test_single_event(watched_dir,
                      ...
                      {
#ifdef _WIN32
                          BOOST_REQUIRE(SetFileAttributesW(to_os_string(test_file).c_str(), FILE_ATTRIBUTE_TEMPORARY));
#else
...
// It is a known problem that travis-ci.org does not properly implement
// IN_DELETE_SELF,
...
BOOST_AUTO_TEST_CASE(file_system_watcher_rename_in_dir)
{
    ...
    Si::spawn_coroutine(
        [&watcher, &test_file_a, &test_file_b, &got_something](Si::spawn_context &yield)
        {
            Si::optional<Si::error_or<ventura::file_notification>> first_event = yield.get_one(Si::ref(watcher));
            BOOST_REQUIRE(first_event);
            Si::optional<Si::error_or<ventura::file_notification>> second_event = yield.get_one(Si::ref(watcher));
