...
namespace
{
	...
	process_output run_process(ventura::async_process_parameters parameters,
	                           ...
	                           ventura::environment_inheritance inheritance)
	{
		...
		ventura::async_process process =
		    ventura::launch_process(parameters, standard_input.read.handle, standard_output.write.handle,
		                            standard_error.write.handle, std::move(environment_variables), inheritance)
		        .move_value();
		...
		ventura::experimental::read_from_anonymous_pipe(io, Si::make_container_sink(result.output),
		                                                std::move(standard_output.read), stopped_polling);
		ventura::experimental::read_from_anonymous_pipe(io, Si::make_container_sink(result.error),
		                                                std::move(standard_error.read), stopped_polling);
...
BOOST_AUTO_TEST_CASE(async_process_unix_which)
{
	...
	process_output result = run_process(parameters, std::vector<std::pair<Si::os_char const *, Si::os_char const *>>(),
...
namespace
{
	    ...
	    std::vector<std::pair<Si::os_char const *, Si::os_char const *>> const additional_variables)
	{
		...
		{
			BOOST_CHECK_NE(std::string::npos, output.output.find(Si::to_utf8_string(variable.first) + '=' +
			                                                     Si::to_utf8_string(variable.second)));
		}
