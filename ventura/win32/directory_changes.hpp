...
namespace ventura
{
	namespace win32
	{
		...
		private:
			...
			// The watch file comes after the threading state so that it will be destroyed
			// first to unblock a possible read operation
		    ...
		    , immovable(make_unique<immovable_state>())
		{
			...
			immovable->read_runner = std::async(std::launch::async, [&dispatcher]
			                                    {
				                                    dispatcher.run();
				                                });
		}
